...
const migrateAndSeed = () => {
  ...
  // 1. Check for columns
  const tableInfo = db.prepare("PRAGMA table_info(challenges)").all() as any[];
  ...
  if (!columns.includes("expires_at")) {
    ...
    db.prepare("ALTER TABLE challenges ADD COLUMN expires_at DATETIME").run();
  }

  if (!columns.includes("cover_image")) {
    console.log("Adding 'cover_image' column...");
    db.prepare("ALTER TABLE challenges ADD COLUMN cover_image TEXT").run();
  }

  if (!columns.includes("requirements")) {
    console.log("Adding 'requirements' column...");
    db.prepare("ALTER TABLE challenges ADD COLUMN requirements TEXT").run(); // Stored as JSON string
  }
  ...
  if (existingDaily) {
    ...
    db.prepare(
      `
        UPDATE challenges 
        SET expires_at = ?, cover_image = ?, requirements = ? 
        WHERE id = ?
      `,
    ).run(tomorrow, coverImage, requirements, (existingDaily as any).id);
  } else {
  ...
  // 4. Seed Tools
  // Check if we need to seed (simple check: if less than 10 tools, re-seed/add more)
  const toolCount = db.prepare("SELECT COUNT(*) as count FROM tools").get() as {
    count: number;
  };

  if (toolCount.count < 10) {
    ...
    const insert = db.prepare(
      "INSERT INTO tools (id, name, description, category, url, icon) VALUES (?, ?, ?, ?, ?, ?)",
    );
    tools.forEach((tool) => {
      insert.run(
        randomUUID(),
