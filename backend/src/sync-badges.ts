...
async function syncBadges(email: string) {
  ...
  try {
    const user = await db.get("SELECT id, badges_json FROM users WHERE email = $1", [email]) as any;
    if (!user) {
    ...
    const completedMilestones = await db.all(`
      ...
      WHERE p.user_id = $1 AND l.level_number IN (12, 28, 50)
    `, [user.id]) as any[];
    ...
    if (updated) {
      await db.run("UPDATE users SET badges_json = $1 WHERE id = $2", [JSON.stringify(currentBadges), user.id]);
...
syncBadges("shivamapatel.edunet@gmail.com").then(() => process.exit(0));
