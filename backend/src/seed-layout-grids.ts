...
async function seedLayoutGrids() {
  ...
  try {
    ...
    const existing = db.prepare("SELECT id FROM learning_topics WHERE slug = ?").get('layout-grids');
    if (!existing) {
      db.prepare(`
        INSERT INTO learning_topics (id, title, slug, description, icon)
        VALUES (?, 'Layout & Grids', 'layout-grids', 'Learn to organize content with structure, balance, and rhythm using grid systems.', 'Layout')
      `).run(topicId);
    } else {
    ...
    addQuiz(addSection(l1, 'quiz', 2, 'TEST', {}), 'What is the primary purpose of a design grid?', ['To add decoration', 'To provide structure and alignment', 'To make the file size smaller'], 1);

    // L2: The 8pt Grid
    const l2 = addLevel(2, 'NORMAL');
    addSection(l2, '8pt-grid', 1, 'READ', [
    ...
    addQuiz(addSection(l2, 'quiz', 2, 'TEST', {}), 'Why is the number 8 commonly used in grid systems?', ['It is a lucky number', 'It is easily divisible and scales well across screens', 'It is the default font size'], 1);

    // L3: Column Systems
    const l3 = addLevel(3, 'NORMAL');
    addSection(l3, 'columns', 1, 'READ', [
    ...
    addQuiz(addSection(l3, 'quiz', 2, 'TEST', {}), 'How many columns are standard in a flexible desktop grid?', ['5 columns', '10 columns', '12 columns'], 2);

    // L4: Gutter & Margin
    const l4 = addLevel(4, 'NORMAL');
    addSection(l4, 'spacing', 1, 'READ', [
    ...
    addQuiz(addSection(l4, 'quiz', 2, 'TEST', {}), 'What is the space between two columns called?', ['Margin', 'Gutter', 'Padding'], 1);

    // L5: Layout Balance
    const l5 = addLevel(5, 'NORMAL');
    addSection(l5, 'balance', 1, 'READ', [
    ...
    addQuiz(addSection(l5, 'quiz', 2, 'TEST', {}), 'What is Asymmetrical Balance?', ['Equal weight on both sides', 'Uneven weight that still feels stable', 'Randomly placed elements'], 1);

    // L6: Whitespace
    const l6 = addLevel(6, 'MEDIUM');
    addSection(l6, 'whitespace', 1, 'READ', [
    ...
    addQuiz(addSection(l6, 'quiz', 2, 'TEST', {}), 'What is a major benefit of using whitespace?', ['It makes the design look unfinished', 'It improves focus and readability', 'It fills up the background'], 1);

    // L7: Responsive Layouts
    const l7 = addLevel(7, 'MEDIUM');
    addSection(l7, 'responsive', 1, 'READ', [
    ...
    addQuiz(addSection(l7, 'quiz', 2, 'TEST', {}), 'What is a "breakpoint"?', ['A point where the screen breaks', 'A specific screen width where the layout changes', 'The end of a work session'], 1);

    // L8: Rule of Thirds
    const l8 = addLevel(8, 'MEDIUM');
    addSection(l8, 'rule-of-thirds', 1, 'READ', [
    ...
    addQuiz(addSection(l8, 'quiz', 2, 'TEST', {}), 'How many sections does the Rule of Thirds divide a layout into?', ['3', '6', '9'], 2);

    // L9: Modular Grids
    const l9 = addLevel(9, 'MEDIUM');
    addSection(l9, 'modular', 1, 'READ', [
    ...
    addQuiz(addSection(l9, 'quiz', 2, 'TEST', {}), 'A Modular grid adds what to a standard column grid?', ['Horizontal rows', 'More colors', 'Larger margins'], 0);
    ...
    addQuiz(addSection(l10, 'quiz', 2, 'TEST', {}), 'Which grid increment is standard for mobile touch targets?', ['8px', '44px or 48px', '100px'], 1);
