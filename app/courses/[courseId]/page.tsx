...
export default async function CoursePlayerPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col pt-24 md:pt-28">
       {/* Top Navigation Bar */}
       <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-4 sticky top-24 md:top-28 z-40 transition-all">
          <Link href="/learning-paths" className="p-2 hover:bg-gray-100 rounded-full transition-colors">
             <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
             <h1 className="font-bold text-lg leading-tight">{activeTitle}</h1>
             <p className="text-xs text-muted-foreground flex items-center gap-1">
                <span className="bg-black text-white px-1.5 py-0.5 rounded text-[10px] font-bold">COURSE</span>
