...
export async function submitCourse(formData: FormData) {
  try {
    ...
    if (typeof thumbValue === 'string') {
      ...
      thumbnailUrl = (formData.get('thumbnailUrl') as string) || "/course-placeholder.jpg";
    }
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, title, description, price, difficulty, duration, category, video_url, instructor_name, tutor_id, thumbnailUrl, 'PENDING');
    ...
    if (modulesJson) {
      ...
      modules.forEach((mod: any, index: number) => {
        insertModule.run(crypto.randomUUID(), id, mod.title, mod.duration, index + 1);
      });
    }
...
export async function getCourseById(id: string) {
  try {
    const course = db.prepare('SELECT * FROM courses WHERE id = ?').get(id) as Course;
    
    if (!course) return null;

    const modules = db.prepare('SELECT * FROM modules WHERE course_id = ? ORDER BY [order]').all(id) as any[];
    
    // Fetch lessons for each module
    const modulesWithLessons = modules.map(mod => {
        const lessons = db.prepare('SELECT * FROM lessons WHERE module_id = ? ORDER BY [order]').all(mod.id);
        return { ...mod, lessons };
    });
