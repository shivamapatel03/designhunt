    ...
    <div className="min-h-screen bg-[#fafafa] pt-36 pb-20 font-[family-name:var(--font-plus-jakarta)]">
      <div className="max-w-4xl mx-auto px-6 space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10 items-start">
          ...
          {/* Right Side: Profile Section (Mini) */}
          <div className="order-1 lg:order-2 lg:sticky lg:top-40">
            <div className="bg-white p-4 rounded-[24px] border border-black/5 border-b-4 border-black/5 flex items-center gap-4">
               <Link href="/profile/edit" className="relative group cursor-pointer shrink-0">
                  <img 
                    src={user.avatar} 
                    className="w-14 h-14 rounded-full border-2 border-white relative z-10 object-cover bg-gray-50 transition-all group-hover:brightness-90 shadow-sm" 
                    alt="Avatar" 
                  />
                  <div className="absolute -bottom-0.5 -right-0.5 bg-black text-white p-1 rounded-full z-20 border border-white shadow-sm transition-transform group-hover:scale-110">
                    <Edit3 className="w-2.5 h-2.5" />
                  </div>
               </Link>
               <div className="min-w-0 flex-1">
                  <h1 className="text-sm font-bold tracking-tight text-black truncate leading-none">
                    {user.name}
                  </h1>
                  <p className="text-[9px] font-bold text-gray-400 leading-none mt-1.5 uppercase tracking-widest">{user.handle}</p>
                  <div className="flex items-center gap-1.5 mt-2.5 text-gray-300 font-medium">
                    <Calendar className="w-3 h-3" />
                    <span className="text-[9px] font-bold leading-none">{today}</span>
                  </div>
               </div>
            </div>
          </div>
